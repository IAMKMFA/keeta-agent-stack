...
export function HeroSection() {
  return (
    <section className="relative isolate overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[680px] opacity-25 [mask-image:radial-gradient(closest-side,black,transparent)]"
      >
        <AgentNetworkHero />
      </div>
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-5 py-20 lg:grid-cols-[1.1fr_0.9fr] lg:py-28">
        <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-md px-5 py-3 font-medium text-zinc-300 transition-colors hover:text-white"
            >
              {heroContent.tertiaryCta.label}
            </Link>
            <Link
              href={siteConfig.repoUrl}
              className="inline-flex items-center gap-2 rounded-md border border-white/10 px-4 py-3 text-sm text-zinc-300 transition-colors hover:border-white/30 hover:text-white"
        ...
        <div className="surface-card relative p-6 shadow-card-elevated">
          ...
          <ol className="mt-4 grid grid-cols-2 gap-3">
            {[
              ...
              <li
                key={title}
                className="rounded-md border border-line bg-black/30 p-4 transition-colors hover:border-keeta/30"
              >
