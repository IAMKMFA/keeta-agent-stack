...
| `@keeta-agent-stack/adapter-x402`                                       | **Simulatable**          | First-class `VenueAdapter` for HTTP 402-gated agent payments. Quotes and `execute(mode='simulate')` return structured agent-payment receipts; live returns a typed not-configured error. |
| `@keeta-agent-stack/adapter-pay-sh`                                     | **Simulatable**          | First-class `VenueAdapter` for pay.sh-style API discovery and payment. Current catalog includes Gemini, BigQuery, and Vertex AI fixtures; live execution is not configured.              |
| `@keeta-agent-stack/adapter-mpp`                                        | **Simulatable**          | First-class `VenueAdapter` for Machine Payments Protocol-style flows. Quote and simulate are present; live execution returns a typed not-configured error.                               |
...
| `apps/web` preset fiat/card/bridge rails                                | **Preset-only mock**     | USD ACH, EUR SEPA, Visa Direct, Base/Solana/Tron corridors, and Wormhole are typed demo fixtures with explicit support levels. They are not adapter packages in this pass.               |
