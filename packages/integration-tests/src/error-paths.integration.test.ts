...
integration('integration error paths', () => {
  ...
  it('marks live execution as failed when the kill switch is active', async () => {
    activeRuntime = await createIntegrationTestRuntime({
      envOverrides: {
  ...
  it('blocks execution when the latest policy decision is disallowed', async () => {
    activeRuntime = await createIntegrationTestRuntime({
      envOverrides: {
  ...
  it('caps webhook retries and leaves permanently failing deliveries in a terminal failed state', async () => {
    activeRuntime = await createIntegrationTestRuntime({
      envOverrides: {
    ...
    const matchingAttempts = activeRuntime.receivedWebhooks.filter(
      (entry) =>
        entry.payload?.eventType === 'execution.completed' && entry.payload?.intentId === intent.id
    );
    expect(matchingAttempts).toHaveLength(2);
  }, 30_000);

  it('marks the intent failed when an adapter returns an execution error', async () => {
    activeRuntime = await createIntegrationTestRuntime({
