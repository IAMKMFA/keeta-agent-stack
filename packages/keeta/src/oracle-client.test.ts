...
describe('KtaOracleClient', () => {
  it('builds rate query and parses JSON', async () => {
    const fetchImpl = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      expect(String(input)).toBe('https://kta-oracle.vercel.app/rate?currency=USD&wallet=keeta_abc');
  ...
  it('throws typed errors for non-2xx responses', async () => {
    const fetchImpl = vi.fn(async () => new Response(JSON.stringify({ error: 'downstream' }), { status: 503 }));
    const client = new KtaOracleClient({ fetchImpl });
  ...
  it('calls MCP tools via JSON-RPC', async () => {
    const fetchImpl = vi.fn(async (_input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
      const body = JSON.parse(String(init?.body)) as { method: string; params: { name: string; arguments: { currency: string } } };
